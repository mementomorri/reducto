"""
Custom test assertions for reducto test suite.
"""
...
def assert_json_structure_equal(actual: Any, expected: Any, path: str = ""):
    ...
    if isinstance(expected, dict):
        if not isinstance(actual, dict):
            raise AssertionError(f"Expected dict at {path}, got {type(actual).__name__}")
        ...
        if not isinstance(actual, list):
            raise AssertionError(f"Expected list at {path}, got {type(actual).__name__}")

        if len(expected) > 0 and len(actual) > 0:
            assert_json_structure_equal(actual[0], expected[0], f"{path}[0]")

    elif isinstance(expected, type):
        if not isinstance(actual, expected):
            raise AssertionError(
                f"Expected type {expected.__name__} at {path}, " f"got {type(actual).__name__}"
            )
...
def assert_git_clean(repo_path: Path):
    ...
    if result.stdout.strip():
        raise AssertionError(f"Git repository has uncommitted changes:\n{result.stdout}")
...
def assert_valid_javascript_syntax(code: str):
    ...
    try:
        result = subprocess.run(
            ["node", "--check", "-"], input=code, capture_output=True, text=True
        )
...
def assert_complexity_metrics_valid(metrics: dict):
    ...
    for field in required_fields:
        ...
        if not isinstance(value, (int, float)):
            raise AssertionError(f"Metric {field} must be numeric, got {type(value).__name__}")
