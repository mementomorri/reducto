...
from reducto.utils import calculate_complexity


class ValidatorAgent:
    def __init__(self, workspace=None):
        self.workspace = workspace
        self._deduplicator: DeduplicatorAgent | None = None
        self._idiomatizer: IdiomatizerAgent | None = None
    ...
    def _find_plan(self, session_id: str) -> RefactorPlan | None:
    ...
    async def _apply_change(self, change: FileChange) -> bool:
        try:
            ...
            with open(change.path, encoding="utf-8") as f:
                content = f.read()
    ...
    async def _run_tests(self) -> bool:
        if self.workspace:
            return self.workspace.run_tests().get("success", False)
